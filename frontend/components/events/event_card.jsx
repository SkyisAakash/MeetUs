  ...
  class EventCard extends React.Component {
    ...
    render(){
      ...
      if (this.props.cardid === "big"){
        ...
        return(
          <Link to={`/events/${this.props.event.id}`} className="cardtemplate">
          <div className="cardtempminidate">{this.datemethod(this.props.event.start_date, "block")}</div>
          <div className="cardtempright">
            <span className="dateline">{this.datemethod(this.props.event.start_date, "line")}</span>
            <h1 className="eventcardShowcuteTitle">{this.props.event.title}</h1>
          </div>
        </Link>
          );
    }
